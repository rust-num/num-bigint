...
fn test_from_biguint() {
    fn check(inp_s: Sign, inp_n: usize, ans_s: Sign, ans_n: usize) {
        let inp = BigIntSmall::from_biguint(inp_s, BigUint::from(inp_n));
        let ans = BigIntSmall {
            sign: ans_s,
...
fn test_from_slice() {
    fn check(inp_s: Sign, inp_n: u32, ans_s: Sign, ans_n: u32) {
        let inp = BigIntSmall::from_slice(inp_s, &[inp_n]);
        let ans = BigIntSmall {
            sign: ans_s,
            data: BigUint::from(ans_n),
        };
...
fn test_assign_from_slice() {
    fn check(inp_s: Sign, inp_n: u32, ans_s: Sign, ans_n: u32) {
        let mut inp = BigIntSmall::from_slice(Minus, &[2627_u32, 0_u32, 9182_u32, 42_u32]);
