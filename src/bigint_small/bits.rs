...
forward_val_val_binop!(impl BitAnd for BigIntSmall, bitand);
forward_ref_val_binop!(impl BitAnd for BigIntSmall, bitand);

// do not use forward_ref_ref_binop_commutative! for bitand so that we can
// clone as needed, avoiding over-allocation
impl<'a, 'b> BitAnd<&'b BigIntSmall> for &'a BigIntSmall {
...
impl<'a> BitAnd<&'a BigIntSmall> for BigIntSmall {
    type Output = BigIntSmall;

    #[inline]
    fn bitand(mut self, other: &BigIntSmall) -> BigIntSmall {
        self &= other;
        self
    }
}

forward_val_assign!(impl BitAndAssign for BigIntSmall, bitand_assign);
...
forward_val_val_binop!(impl BitOr for BigIntSmall, bitor);
forward_ref_val_binop!(impl BitOr for BigIntSmall, bitor);

// do not use forward_ref_ref_binop_commutative! for bitor so that we can
// clone as needed, avoiding over-allocation
impl<'a, 'b> BitOr<&'b BigIntSmall> for &'a BigIntSmall {
    type Output = BigIntSmall;
...
impl<'a> BitOr<&'a BigIntSmall> for BigIntSmall {
    type Output = BigIntSmall;

    #[inline]
    fn bitor(mut self, other: &BigIntSmall) -> BigIntSmall {
        self |= other;
        self
    }
}

forward_val_assign!(impl BitOrAssign for BigIntSmall, bitor_assign);
...
forward_all_binop_to_val_ref_commutative!(impl BitXor for BigIntSmall, bitxor);

impl<'a> BitXor<&'a BigIntSmall> for BigIntSmall {
    type Output = BigIntSmall;

    #[inline]
    fn bitxor(mut self, other: &BigIntSmall) -> BigIntSmall {
        self ^= other;
        self
    }
}

forward_val_assign!(impl BitXorAssign for BigIntSmall, bitxor_assign);
