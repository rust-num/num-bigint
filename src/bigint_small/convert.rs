...
use super::{BigIntSmall, ToBigInt};
...
impl FromStr for BigIntSmall {
    type Err = ParseBigIntError;

    #[inline]
    fn from_str(s: &str) -> Result<BigIntSmall, ParseBigIntError> {
        BigIntSmall::from_str_radix(s, 10)
    }
}

impl Num for BigIntSmall {
    type FromStrRadixErr = ParseBigIntError;

    /// Creates and initializes a BigInt.
    #[inline]
    fn from_str_radix(mut s: &str, radix: u32) -> Result<BigIntSmall, ParseBigIntError> {
        ...
        Ok(BigIntSmall::from_biguint(sign, bu))
    }
}
...
macro_rules! impl_try_from_bigint {
    ($T:ty, $to_ty:path) => {
        #[cfg(has_try_from)]
        impl TryFrom<&BigIntSmall> for $T {
            type Error = TryFromBigIntError<()>;

            #[inline]
            fn try_from(value: &BigIntSmall) -> Result<$T, TryFromBigIntError<()>> {
        ...
        #[cfg(has_try_from)]
        impl TryFrom<BigIntSmall> for $T {
            type Error = TryFromBigIntError<BigIntSmall>;

            #[inline]
            fn try_from(value: BigIntSmall) -> Result<$T, TryFromBigIntError<BigIntSmall>> {
...
impl FromPrimitive for BigIntSmall {
    #[inline]
    fn from_i64(n: i64) -> Option<BigIntSmall> {
        Some(BigIntSmall::from(n))
    }

    #[inline]
    fn from_i128(n: i128) -> Option<BigIntSmall> {
        Some(BigIntSmall::from(n))
    }

    #[inline]
    fn from_u64(n: u64) -> Option<BigIntSmall> {
        Some(BigIntSmall::from(n))
    }

    #[inline]
    fn from_u128(n: u128) -> Option<BigIntSmall> {
        Some(BigIntSmall::from(n))
    }

    #[inline]
    fn from_f64(n: f64) -> Option<BigIntSmall> {
        if n >= 0.0 {
            BigUint::from_f64(n).map(BigIntSmall::from)
        } else {
            let x = BigUint::from_f64(-n)?;
            Some(-BigIntSmall::from(x))
        }
    }
}

impl From<i64> for BigIntSmall {
    #[inline]
    fn from(n: i64) -> Self {
        if n >= 0 {
...
impl From<i128> for BigIntSmall {
    #[inline]
    fn from(n: i128) -> Self {
        if n >= 0 {
...
macro_rules! impl_bigint_from_int {
    ($T:ty) => {
        impl From<$T> for BigIntSmall {
            #[inline]
            fn from(n: $T) -> Self {
                BigIntSmall::from(n as i64)
            }
        }
    };
}
...
impl From<u128> for BigIntSmall {
    #[inline]
    fn from(n: u128) -> Self {
        if n > 0 {
            BigIntSmall {
...
impl From<BigUint> for BigIntSmall {
    #[inline]
...
impl ToBigUint for BigIntSmall {
    #[inline]
...
#[cfg(has_try_from)]
impl TryFrom<&BigIntSmall> for BigUint {
    type Error = TryFromBigIntError<()>;

    #[inline]
    fn try_from(value: &BigIntSmall) -> Result<BigUint, TryFromBigIntError<()>> {
        value
            .to_biguint()
...
impl TryFrom<BigIntSmall> for BigUint {
    ...
    #[inline]
    fn try_from(value: BigIntSmall) -> Result<BigUint, TryFromBigIntError<BigIntSmall>> {
...
#[inline]
pub(super) fn from_signed_bytes_be(digits: &[u8]) -> BigIntSmall {
    let sign = match digits.first() {
        Some(v) if *v > 0x7f => Sign::Minus,
        Some(_) => Sign::Plus,
        None => return BigIntSmall::zero(),
    };
    ...
    if sign == Sign::Minus {
        ...
        BigIntSmall::from_biguint(sign, BigUint::from_bytes_be(&*digits))
    } else {
        BigIntSmall::from_biguint(sign, BigUint::from_bytes_be(digits))
    }
}
...
#[inline]
pub(super) fn from_signed_bytes_le(digits: &[u8]) -> BigIntSmall {
    ...
    if sign == Sign::Minus {
        ...
        BigIntSmall::from_biguint(sign, BigUint::from_bytes_le(&*digits))
    } else {
        BigIntSmall::from_biguint(sign, BigUint::from_bytes_le(digits))
    }
}
